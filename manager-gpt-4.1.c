    ...
    struct rs_state_data *dst_rs_state_data)
{
    ...
    struct rs_ipc_filter_el (*rs_ipc_filter_els)[IPCF_MAX_ELEMENTS] = NULL;
    ...
    if (src_rs_state_data->ipcf_els_size % rs_ipc_filter_size != 0) {
        return E2BIG;
    }
        ...
        for (j = 0; j < IPCF_MAX_ELEMENTS && rs_ipc_filter[j].flags; j++) {
            ...
            if (flags & IPCF_MATCH_M_SOURCE) {
                if (ds_retrieve_label_endpt(rs_ipc_filter[j].m_label, &m_source) != OK) {
                    char *buff = NULL;
                    if (strcmp("ANY_USR", rs_ipc_filter[j].m_label) == 0) {
                        m_source = ANY_USR;
                    } else if (strcmp("ANY_SYS", rs_ipc_filter[j].m_label) == 0) {
                        m_source = ANY_SYS;
                    } else if (strcmp("ANY_TSK", rs_ipc_filter[j].m_label) == 0) {
                        m_source = ANY_TSK;
                    } else {
                        errno = 0;
                        m_source = (endpoint_t)strtol(rs_ipc_filter[j].m_label, &buff, 10);
                        if (errno != 0 || !buff || *buff != '\0') {
                            free(ipcf_els_buff);
                            if (dst_rs_state_data->eval_addr) {
                                free(dst_rs_state_data->eval_addr);
                                dst_rs_state_data->eval_addr = NULL;
...
void build_cmd_dep(struct rproc *rp)
{
    ...
    if (rp == NULL || rp->r_pub == NULL || rp->r_cmd == NULL || rp->r_args == NULL || rp->r_argv == NULL)
        return;

    rpub = rp->r_pub;

    if (strncpy(rp->r_args, rp->r_cmd, ARGS_SIZE - 1) == NULL) {
        rp->r_args[0] = '\0';
        return;
    }
    rp->r_args[ARGS_SIZE - 1] = '\0';
...
void end_srv_init(struct rproc *rp)
{
    ...
    if (rp->r_prev_rp != NULL) {
        ...
        if (rs_verbose) {
            const char *srv_str = srv_to_string(rp);
            if (srv_str != NULL) {
...
int crash_service_debug(char *file, int line, struct rproc *rp)
{
    ...
    if (rs_verbose) {
        const char *exiting_str = (rp->r_flags & RS_EXITING) ? "lethally " : "";
        const char *srv_str = srv_to_string(rp);
        printf("RS: %s %skilled at %s:%d\n", srv_str ? srv_str : "unknown", exiting_str, file, line);
    }
...
void cleanup_service_debug(char *file, int line, struct rproc *rp)
{
    ...
    if (!(rp->r_flags & RS_DEAD)) {
        ...
        if (sys_privctl(rpub->endpoint, SYS_PRIV_DISALLOW, NULL) != OK) {
            printf("RS: warning: failed to disallow priv for %s\n", srv_to_string(rp));
        }
        if (sys_privctl(rpub->endpoint, SYS_PRIV_CLEAR_IPC_REFS, NULL) != OK) {
            printf("RS: warning: failed to clear IPC refs for %s\n", srv_to_string(rp));
        }
...
void detach_service_debug(const char *file, int line, struct rproc *rp)
{
    ...
    if (!file || !rp || !(rpub = rp->r_pub)) {
        fprintf(stderr, "detach_service_debug: invalid argument\n");
        return;
    }
    ...
    if (snprintf(rpub->label, RS_MAX_LABEL_LEN, "%lu.%s", ++detach_counter, label) >= RS_MAX_LABEL_LEN) {
        fprintf(stderr, "detach_service_debug: label truncated\n");
    }
    ...
    if (sys_privctl(rpub->endpoint, SYS_PRIV_ALLOW, NULL) != 0) {
        fprintf(stderr, "detach_service_debug: sys_privctl failed\n");
    }
}
...
int create_service(struct rproc *rp)
{
    ...
    rpub = rp->r_pub;
    use_copy = (rpub->sys_flags & SF_USE_COPY) ? 1 : 0;
    ...
    if (!use_copy && (!rp->r_cmd || !strcmp(rp->r_cmd, ""))) {
    ...
    if (s != 0) {
        printf("RS: unable to get child endpoint: %d\n", s);
        cleanup_service(rp);
        return s;
    }
    ...
    if (rp->r_priv.s_flags & VM_SYS_PROC) {
        ...
        for (i = 0; i < nr_rs_rps; i++) {
            if (rs_rps && rs_rps[i] && rs_rps[i]->r_pub)
...
int clone_service(struct rproc *rp, int instance_flag, int init_flags)
{
    ...
    *replica_link = rp;
    ...
    if ((replica_rp->r_priv.s_flags & rs_flags) == rs_flags) {
        ...
        if (!rs_rp) {
            *rp_link = NULL;
            return kill_service(replica_rp, "RS process pointer NULL", EFAULT);
        }
...
int publish_service(struct rproc *rp)
{
    ...
    if (!rp || !(rp->r_pub)) {
        return kill_service(rp, "invalid rproc or rprocpub", -1);
    }
    ...
    if (rpub->pci_acl.rsp_nr_device || rpub->pci_acl.rsp_nr_class) {
        memset(&pci_acl, 0, sizeof(pci_acl));
    ...
    if (rs_verbose) {
        const char *service_str = srv_to_string(rp);
        if (service_str != NULL) {
...
int unpublish_service(struct rproc *rp)
{
    ...
    if (!rp || !(rp->r_pub)) {
        printf("RS: Invalid service pointer\n");
        return EINVAL;
    }
    ...
    if (rs_verbose) {
        ...
        if (srv_str)
            ...
            printf("RS: service unpublished (invalid service string)\n");
    }
...
void activate_service(struct rproc *rp, struct rproc *ex_rp)
{
    if (ex_rp && (ex_rp->r_flags & RS_ACTIVE)) {
        ...
        if (rs_verbose) {
            const char *ex_rp_str = srv_to_string(ex_rp);
            if (ex_rp_str) {
    ...
    if (!(rp->r_flags & RS_ACTIVE)) {
        rp->r_flags |= RS_ACTIVE;
        if (rs_verbose) {
            const char *rp_str = srv_to_string(rp);
            if (rp_str) {
...
void reincarnate_service(struct rproc *old_rp)
{
    ...
    result = clone_slot(old_rp, &rp);
    if (result != OK || rp == NULL) {
    ...
    if (rp->r_pub && rp->r_pub->endpoint >= 0 &&
        _ENDPOINT_P(rp->r_pub->endpoint) < NR_PROCS && _ENDPOINT_P(rp->r_pub->endpoint) >= 0) {
        rproc_ptr[_ENDPOINT_P(rp->r_pub->endpoint)] = NULL;
    }

    prev_restarts = rp->r_restarts;

    start_service(rp, SEF_INIT_FRESH);

    rp->r_restarts = prev_restarts + 1;
}
...
void terminate_service(struct rproc *rp)
{
    ...
    norestart = !(rp->r_flags & RS_EXITING) && (rpub->sys_flags & SF_NORESTART);
    ...
    if (rp->r_restarts > 0) {
        if (!(rpub->sys_flags & SF_NO_BIN_EXP)) {
            int exponent = MIN(rp->r_restarts, (BACKOFF_BITS - 2));
            rp->r_backoff = 1 << exponent;
...
static int run_script(struct rproc *rp)
{
	...
	if (rp == NULL || rp->r_pub == NULL || rp->r_script == NULL)
		return EINVAL;
	...
	if (snprintf(incarnation_str, sizeof(incarnation_str), "%d", rp->r_restarts) < 0)
		return EINVAL;
	...
	if (pid == -1) {
		...
		fprintf(stderr, "RS: run_script: execle '%s' failed: %s\n",
			rp->r_script, strerror(errno));
		_exit(1);
	}

	if (getprocnr(pid, &endpoint) != 0)
		return errno;
...
void restart_service(struct rproc *rp)
{
    ...
    if (!replica_rp) {
        kill_service(rp, "replica not available after cloning", EFAULT);
        return;
    }
    ...
    if ((rp->r_pub && (rp->r_pub->sys_flags & SF_DET_RESTART))
         && (rp->r_restarts < MAX_DET_RESTART)) {
        rp->r_flags |= RS_CLEANUP_DETACH;
    }

    if (rs_verbose) {
        const char *orig_str = srv_to_string(rp) ? srv_to_string(rp) : "<unknown>";
        const char *replica_str = srv_to_string(replica_rp) ? srv_to_string(replica_rp) : "<unknown>";
        printf("RS: %s restarted into %s\n", orig_str, replica_str);
    }
}
...
void inherit_service_defaults(struct rproc *def_rp, struct rproc *rp)
{
    ...
    if (def_rpub->nr_domain > 0) {
        memcpy(rpub->domain, def_rpub->domain, sizeof(rpub->domain[0]) * def_rpub->nr_domain);
    }
...
void get_service_instances(struct rproc *rp, struct rproc ***rps, int *length) {
    ...
    if (rp->r_prev_rp && nr_instances < 5) instances[nr_instances++] = rp->r_prev_rp;
    if (rp->r_next_rp && nr_instances < 5) instances[nr_instances++] = rp->r_next_rp;
    if (rp->r_old_rp && nr_instances < 5) instances[nr_instances++] = rp->r_old_rp;
    if (rp->r_new_rp && nr_instances < 5) instances[nr_instances++] = rp->r_new_rp;
...
int read_exec(struct rproc *rp) {
    if (rp == NULL || rp->r_argv == NULL || rp->r_pub == NULL || rp->r_pub->label == NULL)
        return EINVAL;
    ...
    while (total_read < rp->r_exec_len) {
        ssize_t bytes_read = read(fd, (char *)rp->r_exec + total_read, rp->r_exec_len - total_read);
        if (bytes_read < 0) {
            int err = errno;
            printf("RS: read_exec: read failed at offset %zd, errno %d\n", total_read, err);
            free_exec(rp);
            close(fd);
            return -err;
        }
    ...
    if (total_read == rp->r_exec_len)
        return OK;

    printf("RS: read_exec: unexpected EOF, read %zd of %zu bytes\n", total_read, rp->r_exec_len);
    free_exec(rp);
    return EIO;
}
...
void free_exec(struct rproc *rp)
{
    ...
    for (slot_nr = 0; slot_nr < NR_SYS_PROCS; slot_nr++) {
        struct rproc *curr = &rproc[slot_nr];
        if ((curr->r_flags & RS_IN_USE) && curr != rp && curr->r_exec == rp->r_exec) {
            has_shared_exec = 1;
            other_rp = curr;
            break;
        }
...
{
    if (rpp == NULL || *rpp == NULL || src_rp == NULL || dst_rp == NULL) {
        return;
    }
...
{
    if (!src_rpp || !dst_rpp || !*src_rpp || !*dst_rpp || !(*src_rpp)->r_pub || !(*dst_rpp)->r_pub) {
        return;
    }
...
struct rproc* lookup_slot_by_label(const char *label)
{
    ...
    for (int slot_nr = 0; slot_nr < NR_SYS_PROCS; ++slot_nr) {
        ...
        if (!(rp->r_flags & RS_ACTIVE) || rp->r_pub == NULL) {
            continue;
        }
        if (strcmp(rp->r_pub->label, label) == 0) {
            return rp;
        }
    }
    return NULL;
...
struct rproc* lookup_slot_by_dev_nr(dev_t dev_nr)
{
    ...
    for (int slot_nr = 0; slot_nr < NR_SYS_PROCS; slot_nr++) {
        struct rproc *rp = &rproc[slot_nr];
        if (!(rp->r_flags & RS_IN_USE) || !rp->r_pub) {
            continue;
        }
        if (rp->r_pub->dev_nr == dev_nr) {
            return rp;
        }
    }
...
struct rproc* lookup_slot_by_domain(int domain)
{
    ...
    for (int slot_nr = 0; slot_nr < NR_SYS_PROCS; slot_nr++) {
        struct rproc *rp = &rproc[slot_nr];
        if (!(rp->r_flags & RS_IN_USE) || rp->r_pub == NULL) {
            continue;
        }
        struct rprocpub *rpub = rp->r_pub;
...
void free_slot(struct rproc *rp)
{
    ...
    int idx = _ENDPOINT_P(rpub->endpoint);
    if (idx >= 0 && idx < MAX_NR_PROCS) {
        rproc_ptr[idx] = NULL;
    }
}
...
void add_backward_ipc(struct rproc *rp, struct priv *privp)
{
    ...
    if (!rp || !privp || !rp->r_pub) return;

    proc_name = rp->r_pub->proc_name;

    for (rrp = BEG_RPROC_ADDR; rrp < END_RPROC_ADDR; rrp++) {
        if (!(rrp->r_flags & RS_IN_USE) || !rrp->r_ipc_list[0] || !rrp->r_pub)
            continue;
...
void init_privs(struct rproc *rp, struct priv *privp)
{
    ...
    if (rp == NULL || privp == NULL || rp->r_ipc_list == NULL) {
        return;
    }
